...
2. **Provenance fields** (`introducedInVersion` / `lastModifiedInVersion` / `supersededInVersion`) + **`unchanged`-text validation rule** (verbatim-carry enforcement, §D).
3. **Diff/base semantics** — recomputed merge-base per run (target-branch decision) + **rebase v1 3-way rule** (§C: append / base-unchanged / base-moved → full re-review).
